...
export default function DrawingOverlay() {
  // * - returns an array of ReadDrawing objects read from the server and dequantized
  ...
  useEffect(() => {
    drawings.forEach((d) => {
      ...
      const offset =
        side === "bottom" ? offsetSeed * 60 + 10 : offsetSeed * 70 + 5;
      ...
      {items.map((it) => {
        ...
        return (
          <div
            key={it.id}
            style={style}
            className="opacity-95 fade-out transition-opacity"
          >
            <svg
              viewBox={`0 0 100 100`}
              width="100%"
